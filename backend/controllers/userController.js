  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      const [[basic]] = await pool.query(`
        SELECT
          (SELECT COUNT(*) FROM users WHERE is_active = TRUE) totalUsers,
          (SELECT COUNT(*) FROM departments) totalDepartments,
          (SELECT COUNT(*) FROM tickets) totalTickets,
          (SELECT COUNT(*) FROM notifications) totalNotifications
      `);
    ...
    // ---------------- EXTERNAL ----------------
      ...
      const [[basic]] = await pool.query(`
        SELECT
          (SELECT COUNT(*) FROM tickets WHERE created_by = ?) tickets,
          (SELECT COUNT(*) FROM tickets WHERE created_by = ? AND status_id = 2) closed,
          (SELECT COUNT(*) FROM notifications WHERE created_by = ?) notifications
      `, [userId, userId, userId]);

      const [rows] = await pool.query(`
        SELECT
            n.notification_id,
            COUNT(DISTINCT nm.department_id) total_department_count,
            COALESCE(
                SUM(CASE WHEN nm.is_acknowledged=TRUE THEN 1 ELSE 0 END),
                0
            ) acknowledged_department_count
      ...
      rows.forEach(r => {
        const total = Number(r.total_department_count);
        const ack = Number(r.acknowledged_department_count);
