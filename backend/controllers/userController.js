...
async function getUsers(req,res){
    try{
        const [rows] = await pool.query(`select user_id, username, role, department_id, is_active from users`);
        res.json(rows);
    }
...
async function createUser(req,res){
    try{
        ...
        await pool.query(
`insert into users(username, role, department_id, created_by) values(?,?,?,?)`,
[username, role, department_id, created_by]);
