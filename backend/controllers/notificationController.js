...
let query = `
select n.notification_id, n.title, n.description, n.created_at,
u.username, group_concat(d.department_name separator ',') as departments
...
const [rows] = await pool.query(
`SELECT n.notification_id, n.title, n.description, n.created_at, n.is_acknowledged,
u.username AS created_by
FROM notifications n
JOIN notification_master nm ON n.notification_id = nm.notification_id
LEFT JOIN users u ON n.created_by = u.user_id
WHERE n.notification_id = ? AND nm.department_id = ?`,
...
const departmentId = req.user.department_id;
const [rows] = await pool.query(
`SELECT DISTINCT n.notification_id, n.title, n.description, n.created_at, n.is_acknowledged,
u.username AS created_by
FROM notifications n
JOIN notification_master nm ON n.notification_id = nm.notification_id
LEFT JOIN users u ON n.created_by = u.user_id
...
JOIN notification_master nm ON n.notification_id = nm.notification_id
SET n.is_acknowledged = TRUE, n.acknowledged_by = ?, n.acknowledged_at = NOW()
WHERE n.notification_id = ? AND nm.department_id = ?`,
