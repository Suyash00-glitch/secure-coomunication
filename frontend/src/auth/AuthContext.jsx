...
export function AuthProvider({ children }) {
  ...
  function logout() {
    // Best-effort activity log entry — fired while the token is still
    // valid, before it's cleared below. Internal auth is otherwise fully
    // stateless, so this is the only backend touchpoint logout has; it
    // must never block or fail the actual client-side logout.
    if (user?.role === "secure") {
      apiFetch("/api/internal/logout", { method: "POST" }).catch(() => {});
    }
    disconnectSocket(); 
