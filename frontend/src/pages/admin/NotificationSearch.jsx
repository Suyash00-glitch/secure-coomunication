...
const paginatedNotifications = filtered.slice(
  ...
  <span className="info-key">Acknowledged</span>
  <span style={{ fontWeight: 500, color: "#374151" }}>
    {detail.acknowledged_departments?.length || 0} / {departments.length} departments
  </span>
</div>
...
{departments.length > 0 && (
  <div style={{ marginTop: 12 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {detail.acknowledged_departments?.map((dep, idx) => (
        <div key={idx} style={{
          display: "flex",
          alignItems: "center",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontWeight: 500 }}>{dep.department_name}</span>
          </div>
          <div style={{ color: "#6b7280", fontSize: 12, textAlign: "right" }}>
            <span>{dep.acknowledged_by}</span>
            <span style={{ margin: "0 6px" }}>•</span>
            <span>{fmtDateTime(dep.acknowledged_at)}</span>
          </div>
        </div>
      ))}
      {departments
        .filter(d => !detail.acknowledged_departments?.find(a => a.department_name === d))
        .map((d, idx) => (
          <div key={idx} style={{
            display: "flex",
            alignItems: "center",
            ...
            <span>{d}</span>
            <span style={{ marginLeft: "auto", fontSize: 12 }}>Pending</span>
          </div>
        ))
      }
    </div>
      ...
      <div className="card">
        ...
        <div className="pag-row">
          <span style={{ fontSize: 12, color: "#6b7280" }}>
            {loading ? "Loading..." :`Showing ${paginatedNotifications.length} of ${filtered.length} records`}
          </span>
