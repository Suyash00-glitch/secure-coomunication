...
export default function NotificationDetail() {
  ...
  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
    {departments.map(d => (
      <span className="badge badge-open" key={d}>{d}</span>
    ))}
  </div>
</div>
...
<div className="info-row">
  ...
  <span style={{ fontWeight: 500, color: "#374151" }}>
    {nd.acknowledged_departments?.length || 0} / {departments.length} departments
  </span>
</div>

{nd.acknowledged_departments?.length > 0 && (
  <div style={{ marginTop: 12 }}>
    <div style={{ fontSize: 12, color: "#6b7280", fontWeight: 500, marginBottom: 8 }}>
      Acknowledgement Details
    </div>
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {nd.acknowledged_departments.map((dep, idx) => (
        <div key={idx} style={{
          display: "flex",
          alignItems: "center",
      ...
      {departments
        .filter(d => !nd.acknowledged_departments.find(a => a.department_name === d))
        .map((d, idx) => (
          <div key={idx} style={{
            display: "flex",
            alignItems: "center",
