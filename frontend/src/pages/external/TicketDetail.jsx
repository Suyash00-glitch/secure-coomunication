...
function addCustomValue() {
 ...
 async function sendReply() {
  ...
  if (responses.length === 0) {
    alert("At least one response option required");
    return;
  }
  ...
  try {
    ...
    if (attachment) {
      ...
      const uploadRes = await fetch(
        ...
        `http://localhost:3000/api/tickets/${id}/attachment`,
      ...
      if (!uploadRes.ok) {
        alert("Reply sent, but attachment upload failed.");
      }
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <div className="card-header">
             ...
             <span className={`badge ${statusBadgeClass(td.display_status)}`}>
  {td.display_status}
</span>
            </div>
  ...
  <label className="form-label">
    Suggested Responses For Internal Team
  </label>
  ...
  <div className="response-options">
    ...
    {RESPONSE_OPTIONS.map(option => {
      ...
      const added = expectedResponses.includes(option);

      return (

        <div
          key={option}
          className={added ? "option-item option-added" : "option-item"}
        >
  ...
  <div className="response-selected">
    ...
    <div className="selected-list">
      ...
      {expectedResponses.map((item,index)=>(
...
<button className="btn-ack" onClick={sendReply}>
Send Reply
</button>
</div>
            </div>
