...
export default function CreateNotification() {
  ...
  async function createNotify() {
     if (!notifTitle.trim()) { alert("Notification title is required"); return; }
  if (!notifDescription.trim()) { alert("Notification description is required"); return; }
  if (getDepartmentValues().length === 0) { alert("Please select at least one department"); return; }
    setSubmitting(true);
    try {
    ...
    await fetch(
        ...
        `http://localhost:3000/api/notifications/${data.notification_id}/attachment`,
  ...
  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
    ...
    <div>
      ...
      <div className="page-sub">Publish a notification to one or more departments</div>
    </div>
  </div>
</div>
