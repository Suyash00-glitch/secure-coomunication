...
/**
 * Dashboard — fetches stats from backend.
 * TODO: Confirm exact dashboard stats endpoint shape with backend.
 * Expected: GET /api/dashboard/stats → { totalTickets, open, underProcess, closed, notifications, recentActivity, statusSummary }
 */
export default function Dashboard({ user, onNavigate }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        // TODO: Replace with actual dashboard stats endpoint when available
...
const data = await res.json();
  ...
  if (loading) {
    return (
      <div className={true ? "page active" : "page"} id="page-dashboard">
  ...
  return (
    <div className={true ? "page active" : "page"} id="page-dashboard">
      <div className="page-header">
        ...
        <button className="btn btn-primary" id="button-4" onClick={() => onNavigate("create-ticket")}>
      ...
      <div className="qa-grid">
        <div className="qa-card" onClick={() => onNavigate("create-ticket")}><i className="ti ti-plus-square"></i><span>Create Ticket</span></div>
        <div className="qa-card" onClick={() => onNavigate("ticket-list")}><i className="ti ti-list"></i><span>View My Tickets</span></div>
        <div className="qa-card" onClick={() => onNavigate("create-notif")}><i className="ti ti-speakerphone"></i><span>Send Notification</span></div>
        <div className="qa-card" onClick={() => onNavigate("notif-list")}><i className="ti ti-bell-ringing"></i><span>View Notifications</span></div>
      </div>
              ...
              return (
                <>
                  <div className="progress-row">
                    ...
                    <div className="progress-bar-wrap"><div className="progress-bar-fill" style={{ width: `${pct(s.open || 0)}%`, background: "#1a73e8" }}></div></div>
                    <span className="progress-count">{s.open ?? 0}</span>
                  </div>
                  <div className="progress-row">
                    <span className="progress-label">Under Process</span>
                    <div className="progress-bar-wrap"><div className="progress-bar-fill" style={{ width: `${pct(s.underProcess || 0)}%`, background: "#f59e0b" }}></div></div>
                    <span className="progress-count">{s.underProcess ?? 0}</span>
                  </div>
                  <div className="progress-row">
                    <span className="progress-label">Closed</span>
                    <div className="progress-bar-wrap"><div className="progress-bar-fill" style={{ width: `${pct(s.closed || 0)}%`, background: "#22c55e" }}></div></div>
                    <span className="progress-count">{s.closed ?? 0}</span>
                  </div>
                </>
              );
            })()}
